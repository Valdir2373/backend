...
function isLogged(req: CustomRequest, res: Response, next: NextFunction) {
  ...
  if (typeof process.env.JWT_KEY_SECRET === "string")
    jwt.verify(
      token,
      process.env.JWT_KEY_SECRET as Secret,
      (err: any, decoded: any) => {
        // Type Assertion para Secret
        if (err) {
          console.log("aqui: " + token);
...
routerAuth.get(
  ...
  async (req: Request, res: Response) => {
    if (req.user) {
      ...
      // const user = await userServiceAuth.getUserDataByEmail(
      //   (req.user as Profile)
      // );

      //NÃO ESQUECER DE ARRUMAR!!!
